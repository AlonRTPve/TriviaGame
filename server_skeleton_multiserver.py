...
def handle_answer_message(conn, username, data):
    ...
    for k, v in questions.items():
        print((v["correct"]), f"answer is {answer}")
        if k == question_number:
            if v["correct"] == answer:
                build_and_send_message(conn, "CORRECT_ANSWER", "")
                login_system.update_score(username)
                return
            else:
                for k, v in questions.items():
                    if k == question_number:
                        build_and_send_message(conn, chatlib.PROTOCOL_SERVER["WRONG_ANSWER"], str(v["correct"]))
                        return
...
def handle_login_message(conn, data):
    ...
    if login_system.login(user, password):
...
def handle_register_message(conn, data):
    list_user = chatlib.split_data(data, 1)
    user, password = list_user[0], list_user[1]
    if login_system.register(user, password):
